...
export default function AlunoScreen() {
  ...
  useEffect(() => {
    ...
    axios.get(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${estadoSelecionado.id}/municipios`)
      .then(response => setCidades(response.data.map((c: any) => ({ id: c.id, nome: c.nome }))))
  ...
  const limparCampos = () => {
    setNome(''); setMatricula(''); setCurso(''); setEmail(''); setTelefone('');
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <CustomInput
        label="CEP"
        value={cep}
        onChangeText={(text) => { setCep(text); buscarCEP(text); }}
        keyboardType="numeric"
      />
      <CustomInput label="Endereço" value={endereco} onChangeText={setEndereco} />

      <Text style={styles.label}>Estado</Text>
      {carregandoEstados ? <ActivityIndicator /> : (
        <Picker selectedValue={estado} onValueChange={(value) => setEstado(value)} style={styles.picker}>
          <Picker.Item label="Selecione..." value="" />
          {estados.map(e => <Picker.Item key={e.id} label={e.nome} value={e.sigla} />)}
        </Picker>
      )}

      <Text style={styles.label}>Cidade</Text>
      {carregandoCidades ? <ActivityIndicator /> : (
        <Picker selectedValue={cidade} onValueChange={(value) => setCidade(value)} style={styles.picker}>
          <Picker.Item label="Selecione..." value="" />
          {cidades.map(c => <Picker.Item key={c.id} label={c.nome} value={c.nome} />)}
        </Picker>
      )}
